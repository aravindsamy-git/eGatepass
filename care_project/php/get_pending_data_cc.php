...
$sql = "SELECT eGatepass_info.*, student_info.username, student_info.batch, student_info.dept, student_info.h_d, student_info.phone_no 
        ...
        WHERE ((eGatepass_info.status = 'pending' AND student_info.h_d = 'd') OR (eGatepass_info.status = 's2approve' 
        AND student_info.h_d = 'h')) AND (student_info.incharge = '$empid' OR student_info.AIncharge = '$empid');";
